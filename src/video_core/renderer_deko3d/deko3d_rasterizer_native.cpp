...
u32 ColorWriteMask(const Pica::FramebufferRegs& framebuffer) {
    ...
    u32 mask = 0;
    mask |= framebuffer.output_merger.red_enable ? DkColorMask_R : 0;
    mask |= framebuffer.output_merger.green_enable ? DkColorMask_G : 0;
    mask |= framebuffer.output_merger.blue_enable ? DkColorMask_B : 0;
    mask |= framebuffer.output_merger.alpha_enable ? DkColorMask_A : 0;
    return mask;
}
...
bool Rasterizer::InitializeGpuResources() {
    ...
    vertex_gpu_addr = vertex_mem_block ? dkMemBlockGetGpuAddr(vertex_mem_block) : 0;
    ...
    uniform_gpu_addr = uniform_mem_block ? dkMemBlockGetGpuAddr(uniform_mem_block) : 0;
...
Rasterizer::DepthSurface* Rasterizer::GetOrCreateDepthSurface() {
    ...
    DepthKey key{
        ...
        .format = framebuffer.framebuffer.depth_format,
    };
...
bool Rasterizer::SubmitHardwareChunk(FrameContext& context,
    ...
    color_state.logicOp = MapLogicOp(regs.framebuffer.output_merger.logic_op);
    color_state.alphaCompareOp = regs.framebuffer.output_merger.alpha_test.enable != 0
                                    ? MapCompare(regs.framebuffer.output_merger.alpha_test.func)
                                    : DkCompareOp_Always;
    ...
    dkBlendStateSetFactors(&blend_state, MapBlendFactor(blend.factor_source_rgb),
                           MapBlendFactor(blend.factor_dest_rgb),
                           MapBlendFactor(blend.factor_source_a),
                           MapBlendFactor(blend.factor_dest_a));
    ...
    depth_state.depthWriteEnable =
        ...
        regs.framebuffer.output_merger.depth_write_enable != 0;
    depth_state.depthCompareOp = MapCompare(regs.framebuffer.output_merger.depth_test_func);
    depth_state.stencilTestEnable = regs.framebuffer.output_merger.stencil_test.enable != 0;
    depth_state.stencilFrontCompareOp =
        MapCompare(regs.framebuffer.output_merger.stencil_test.func);
    depth_state.stencilBackCompareOp = depth_state.stencilFrontCompareOp;
    depth_state.stencilFrontFailOp =
        MapStencilAction(regs.framebuffer.output_merger.stencil_test.action_stencil_fail);
    depth_state.stencilFrontDepthFailOp =
        MapStencilAction(regs.framebuffer.output_merger.stencil_test.action_depth_fail);
    depth_state.stencilFrontPassOp =
        MapStencilAction(regs.framebuffer.output_merger.stencil_test.action_depth_pass);
...
bool Rasterizer::SubmitTransfer(const DkImageView& source, u32 source_width, u32 source_height,
    ...
    FlushQueueIfNeeded(false);
    return !QueueHasError("after native transfer submit");
}
...
#endif
    // Strict native mode: unsupported state is skipped rather than rasterized on the CPU.
    vertex_batch.clear();
}
...
bool Rasterizer::AccelerateDisplayTransfer(const Pica::DisplayTransferConfig& config) {
    ...
    State::RenderTargetKey destination_key{
        ...
        .format = static_cast<u32>(config.output_format.Value()),
    };
    auto* const destination = state.GetOrCreateRenderTarget(destination_key);
    if (!destination) {
        return false;
    }
    u32 flags = config.flip_vertically ? DkBlitFlag_FlipY : 0;
    if (config.scaling != Pica::DisplayTransferConfig::NoScale) {
        flags |= DkBlitFlag_FilterLinear;
    } else {
        flags |= DkBlitFlag_FilterNearest;
    }
    const bool completed =
        SubmitTransfer(source->view, source->key.width, source->key.height, *destination, flags);
    RecordPicaDisplayTransfer(static_cast<u64>(output_width) * output_height * 4, completed);
    return completed;
#else
    (void)config;
    return false;
...
#ifdef __SWITCH__
    const bool completed = AccelerateDisplayTransfer(config);
    RecordPicaTextureCopy(config.texture_copy.size.Value(), completed);
    return completed;
#else
    (void)config;
    return false;
...
bool Rasterizer::AccelerateFill(const Pica::MemoryFillConfig& config) {
#ifdef __SWITCH__
    const auto* const cached = state.FindGpuDirtyRenderTarget(config.GetStartAddress());
    if (!cached) {
        return false;
    }
    auto* target = state.GetOrCreateRenderTarget(cached->key);
    FrameContext& context = CurrentFrameContext();
    ...
    if (config.fill_32bit) {
        const float r = (config.value_32bit & 0xFFU) / 255.0f;
        const float g = ((config.value_32bit >> 8) & 0xFFU) / 255.0f;
        const float b = ((config.value_32bit >> 16) & 0xFFU) / 255.0f;
        const float a = ((config.value_32bit >> 24) & 0xFFU) / 255.0f;
        dkCmdBufClearColorFloat(command_buffer, 0, DkColorMask_RGBA, r, g, b, a);
    } else {
        dkCmdBufClearColorUint(command_buffer, 0, DkColorMask_RGBA, config.value_16bit.Value(), 0,
                               0, 0xFFFFFFFFU);
    }
    dkCmdBufSignalFence(command_buffer, &context.fence, false);
    const DkCmdList list = dkCmdBufFinishList(command_buffer);
    ...
    FlushQueueIfNeeded(false);
    const bool completed = !QueueHasError("after native fill submit");
    RecordPicaMemoryFill(config.GetEndAddress() - config.GetStartAddress(), completed);
    return completed;
#else
    (void)config;
    return false;
...
bool Rasterizer::AccelerateDrawBatch(bool is_indexed) {
    // Vertex programs still execute through the existing PICA frontend. Their final OutputVertex
    // triangles are consumed by AddTriangle/DrawTriangles and rasterized entirely by Deko3D.
    (void)is_indexed;
    return false;
}
