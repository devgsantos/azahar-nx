...
void AddressSpace::DumpDisassembly() const {
    for (u32* ptr = mem.ptr(); ptr < code.wptr<u32*>(); ptr++) {
        const auto offset = reinterpret_cast<std::uintptr_t>(ptr) -
                            reinterpret_cast<std::uintptr_t>(mem.ptr());
        const auto host_pc = reinterpret_cast<u32*>(reinterpret_cast<std::uintptr_t>(mem.xptr()) +
                                                    offset);
        std::printf("%s", Common::DisassembleAArch64(*ptr, mcl::bit_cast<u64>(host_pc)).c_str());
    }
}
